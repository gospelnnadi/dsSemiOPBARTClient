...
#' @param warmup_burn  number of warm-start iterations run BEFORE the
#'   sync_every-block main loop, communicating every iteration
#'   (sync_every = 1) via ds.semiOPBARTWarmupBlockE() -- same warm-start
#'   ds.semiOPBARTTrainEImproved() uses. Set to 0 to skip.
#' @export
...
#    # PERFORMANCE: DSI renders a progress bar on EVERY datashield.aggregate()
#   # call by default (option "datashield.progress", default TRUE -- see
#   # https://wiki.datashield.org/en/opmanag/indication_of_call_progress).
#   # At num_burn + num_save sweeps x 2-3 aggregate() calls each, that's
#   # thousands of progress-bar renders to the console -- almost certainly
#   # the actual cause of an RStudio console freeze on a run like this, well
#   # before any real computation cost. Save/restore so this doesn't leak
#   # into the caller's session either way (including on error, via on.exit).
#   if (quiet) {
...
#   theta <- rep(1, p)
#   us <- 0:(J - 2); us[1] <- 0
#   message(sprintf("[ds.semiOPBARTTrainE] Initialized with p=%d, J=%d, theta=0, us=%s", p, J, paste(us, collapse = ",")))
